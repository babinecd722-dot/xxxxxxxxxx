...
### Регистрация шаг 1 — пароль + email (screenId=38, t=1)
```json
{"t": 1, "p": "ПАРОЛЬ", "s": ""}
```
- `"p"` — пароль
- `"s"` — email (можно пустой)
- Ник не передаётся в JSON — задаётся как SA-MP ник при подключении
- Сервер ответит `{"t": 3}` → переход к выбору пола

### Регистрация шаг 2 — выбор пола (screenId=38, t=3)
```json
{"t": 3, "r": 0}
```
- `"r"`: 0 = мужской, 1 = женский
- Сервер ответит `{"t": 0}` → переход к инвайту

### Регистрация шаг 3 — ник пригласившего / пропуск (screenId=38, t=4)
```json
{"t": 4, "s": ""}
```
- `"s"` — ник пригласившего (пустая строка = пропустить)
- Сервер ответит `{"t": 0}` → переход к выбору скина

### Регистрация шаг 4 — выбор скина (screenId=38, t=5)
```json
{"t": 5, "r": 78}
```
- `"r"` — gameId скина из списка reg_skin_data
- Сервер завершит регистрацию и ответит `{"t": -1}`

### Порядок бота при регистрации (GUI 38)
```
1. Подключиться → сервер показывает GUI 38 ({"o":1,"r":0,...})
2. Отправить {"t":1, "p":"пароль", "s":""} на guiid 38
3. Получить {"t":3} → отправить {"t":3, "r":0} (муж) на guiid 38
...
4. Шаг 1 — пароль + email:
   КЛИЕНТ: {"t":1, "p":"ПАРОЛЬ", "s":""}  → sendJsonData(38, ...)
   (p = пароль, s = email, можно пустой; ник задаётся при SA-MP подключении)

5. Сервер: {"t":3}  → перейти к выбору пола

6. Шаг 2 — выбор пола:
   КЛИЕНТ: {"t":3, "r":0}  → мужской пол (r=1 = женский)
   → sendJsonData(38, ...)

7. Сервер: {"t":0}  → OK, перейти к инвайту

8. Шаг 3 — ник пригласившего (пропуск):
   КЛИЕНТ: {"t":4, "s":""}  → пустая строка = пропустить
   → sendJsonData(38, ...)

9. Сервер: {"t":0}  → OK, перейти к выбору скина

10. Шаг 4 — выбор скина:
    КЛИЕНТ: {"t":5, "r":78}  → выбрать скин 78
    → sendJsonData(38, ...)
    Мужские скины: [78, 79, 134, 136, 230, 246, 159, 71, 256]
    Женские скины: [77, 135, 188, 212, 239, 218]
